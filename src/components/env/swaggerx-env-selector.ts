...
/**
 * <swaggerx-env-selector> — Dropdown to select the active environment.
 *
 * Fires:
 *   - `env-select` with { id: string | null }
 *   - `env-manage` (open environment manager)
 */
@customElement('swaggerx-env-selector')
export class SwaggerXEnvSelector extends LitElement {
  ...
  override render() {
    return html`
      ...
      <button class="manage-btn" @click=${this._onManage} aria-label="Manage environments">
        <swaggerx-icon name="globe" size=${14}></swaggerx-icon>
      </button>
    `;
  }
}
...
declare global {
  interface HTMLElementTagNameMap {
    'swaggerx-env-selector': SwaggerXEnvSelector;
  }
}
