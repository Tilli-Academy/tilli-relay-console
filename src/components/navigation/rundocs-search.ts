...
/**
 * <rundocs-search> — Fuzzy search input for filtering endpoints.
 *
 * Fires `search-input` with { query: string } on input.
 * Fires `search-clear` when the clear button is clicked.
 */
@customElement('rundocs-search')
export class RunDocsSearch extends LitElement {
  ...
  override render() {
    return html`
      <div class="search-wrapper">
        <rundocs-icon class="search-icon" name="search" size=${16}></rundocs-icon>
        <input
          type="text"
          .value=${this.value}
        ...
        ${this._hasValue
          ? html`
              <button class="clear-btn" @click=${this._onClear} aria-label="Clear search">
                <rundocs-icon name="close" size=${14}></rundocs-icon>
              </button>
            `
          : ''}
      </div>
