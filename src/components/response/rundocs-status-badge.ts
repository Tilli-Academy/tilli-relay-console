...
/**
 * <rundocs-status-badge> — Colored HTTP status code badge.
 *
 * Colors:
 *   2xx → green, 3xx → blue, 4xx → amber, 5xx → red, 0 → gray (error)
 */
@customElement('rundocs-status-badge')
export class RunDocsStatusBadge extends LitElement {
...
declare global {
  interface HTMLElementTagNameMap {
    'rundocs-status-badge': RunDocsStatusBadge;
  }
}
