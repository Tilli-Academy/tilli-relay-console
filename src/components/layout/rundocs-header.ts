...
import '../shared/rundocs-tooltip.js';

/**
 * <rundocs-header> — Top bar with logo, API info, env selector, and theme toggle.
 *
 * Fires:
 *   - `theme-toggle` when theme button is clicked
 *   - `sidebar-toggle` when sidebar toggle is clicked
 */
@customElement('rundocs-header')
export class RunDocsHeader extends LitElement {
  ...
  override render() {
    return html`
      <header>
        <div class="left">
          <rundocs-tooltip text="Toggle sidebar">
            <button class="sidebar-btn" @click=${this._toggleSidebar} aria-label="Toggle sidebar">
              <rundocs-icon name="sidebar-left" size=${18}></rundocs-icon>
            </button>
          </rundocs-tooltip>
          <span class="logo">RunDocs</span>
          ${this.apiTitle
            ? html`
