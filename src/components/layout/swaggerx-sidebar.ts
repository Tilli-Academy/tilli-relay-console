...
/**
 * <swaggerx-sidebar> — Left navigation panel with tabs for endpoints and history.
 *
 * Fires `panel-change` with { panel: 'endpoints' | 'history' }.
 */
@customElement('swaggerx-sidebar')
export class SwaggerXSidebar extends LitElement {
  ...
  override render() {
    return html`
      <div class="sidebar-tabs" role="tablist">
        ...
        >
          <swaggerx-icon name="code" size=${16}></swaggerx-icon>
          Endpoints
        </button>
        <button
          class="sidebar-tab"
        ...
        >
          <swaggerx-icon name="clock" size=${16}></swaggerx-icon>
          History
        </button>
      </div>
...
declare global {
  interface HTMLElementTagNameMap {
    'swaggerx-sidebar': SwaggerXSidebar;
  }
}
