...
import '../../../../src/components/navigation/rundocs-toc.js';
import type { RunDocsToc } from '../../../../src/components/navigation/rundocs-toc.js';
...
describe('rundocs-toc', () => {
  let el: RunDocsToc;

  beforeEach(async () => {
    el = await fixture<RunDocsToc>(html`
      <rundocs-toc .info=${sampleInfo} .servers=${sampleServers}></rundocs-toc>
    `);
  });
  ...
  it('renders nothing when info is null', async () => {
    el = await fixture<RunDocsToc>(html`<rundocs-toc></rundocs-toc>`);
  ...
  it('renders without servers', async () => {
    el = await fixture<RunDocsToc>(html`
      <rundocs-toc .info=${sampleInfo} .servers=${[]}></rundocs-toc>
    `);
    expect(el.shadowRoot!.querySelector('.servers')).toBeNull();
